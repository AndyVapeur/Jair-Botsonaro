...
def send_tweet (sample,client) 
  5.times do |i|
    client.update " Nous en voulez pas on apprend à coder avec @thehackingproject #{sample[i]}"
  end
end

def like_tweets (client)
  tweets_to_like = client.search("#bonjour", result_type: "recent").take(2)
  client.favorite(tweets_to_like)
end

def follow_users (client)
  client.search("#sport", result_type: "mixed").take(10).each do |tweet|
  #accounts_to_follow.map!{|tweet| tweet = tweet.user}
  client.follow(tweet.user)
end
end

def stream (client,login_twitter_streaming)
  login_twitter_streaming.filter(:track => "#covid") do |tweet|
...
def perform
  ...
  login_twitter_streaming
  stream(client,login_twitter_streaming)
end
